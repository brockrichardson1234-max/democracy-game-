/** Pure, synchronous SHA-256 for deterministic content ranking in every host. */
export const sha256Hex = (text: string): string => {
  const encoded: number[] = [];
  for (const character of text) {
    const codePoint = character.codePointAt(0);
    if (codePoint === undefined) continue;
    if (codePoint < 0x80) encoded.push(codePoint);
...
/** Stable bounded value derived from a named deterministic key. */
export const deterministicUnit = (key: string): number =>
  Number.parseInt(sha256Hex(key).slice(0, 8), 16) / 0xffff_ffff;
