...
const git = (...args) => {
  ...
  if (result.error !== undefined || result.status !== 0) {
    ...
    throw new Error(`POP0-I3 ancestry check could not run git ${args.join(" ")}: ${detail}`);
  }
...
if (mergeBase !== acceptedProductionBase) {
  throw new Error(`POP0-I3 merge base must remain ${acceptedProductionBase}; received ${mergeBase}.`);
}
const mainTip = git("rev-parse", "origin/main");
if (mainTip !== acceptedProductionBase) {
  throw new Error(`POP0-I3 verification requires unchanged main at ${acceptedProductionBase}; received ${mainTip}.`);
}
...
if (stageOneAncestry.error !== undefined || ![0, 1].includes(stageOneAncestry.status ?? -1)) {
  ...
  throw new Error(`POP0-I3 Stage 1 ancestry check failed to execute: ${detail}`);
}
if (stageOneAncestry.status === 0) {
  throw new Error(`POP0-I3 branch must not contain Stage 1 commit ${stageOneCommit}.`);
}
...
if (!existsSync(entry)) throw new Error("POP0-I3 proof factory entry is missing.");
...
const visit = async (file) => {
  ...
  if (forbiddenFiles.has(path)) throw new Error(`POP0-I3 reaches forbidden legacy/later dependency ${path}.`);
  const source = await readFile(file, "utf8");
  for (const symbol of forbiddenSymbols) {
    if (source.includes(symbol)) throw new Error(`POP0-I3 reaches forbidden symbol ${symbol} through ${path}.`);
  }
  for (const pattern of forbiddenAuditPatterns) {
    if (pattern.test(source)) throw new Error(`POP0-I3 reaches an audit-only API through ${path}.`);
  }
  for (const pattern of forbiddenActionDispatchPatterns) {
    if (pattern.test(source)) throw new Error(`POP0-I3 reaches action-prefix dispatch through ${path}.`);
  }
...
if (!edges.some((edge) =>
  ...
  throw new Error("POP0-I3 must reuse the narrow lower-level ControlBinding directly.");
}
...
for (const projectionInput of [
  ...
  if (match === null) throw new Error(`POP0-I3 narrow projection input ${projectionInput} is missing.`);
  if (/PresidentialOperatingRuntimeState|PresidentialInterventionState|ownerStates|getOperatingState/.test(match[0])) {
    throw new Error(`POP0-I3 projection ${projectionInput} accepts a full canonical-state graph.`);
  }
}
...
console.log(
  `POP0-I3 boundary verified across ${visited.size} modules at accepted base ${acceptedProductionBase}: direct lower-level ControlBinding only; no Stage 1, legacy session, global action surface, later domain owner, player-facing full state, or audit dependency.`,
);
