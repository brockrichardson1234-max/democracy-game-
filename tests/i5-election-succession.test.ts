...
const allResolutions = (ticketId: string) => US_V0_I4_RUNTIME_ARTIFACTS.populationCohorts.cohorts.map((cohort) => ({
  cohortId: cohort.id,
...
describe("I5 Population-backed selection, declaration, and succession", () => {
  ...
  it("uses elector records when national popular plurality and electoral winner diverge", () => {
    ...
    const resolutions = US_V0_I4_RUNTIME_ARTIFACTS.populationCohorts.cohorts.map((cohort) => ({
      cohortId: cohort.id,
