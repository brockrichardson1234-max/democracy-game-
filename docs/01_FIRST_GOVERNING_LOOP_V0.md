# 01 — First Governing Loop V0

Status: **Commit-1 architecture candidate for review. Not implementation authority.**

## 1. Purpose

Governing Loop 0 is the consumer of Architecture V0.

The architecture does not begin by asking, "What objects exist in government?" It begins by asking:

> What must be true for one complete term of governing to feel causally real, politically constrained, strategically legible, and persistent after the player leaves office?

Every later architecture concept must either serve this loop directly or prevent a foreseeable destructive rewrite.

## 2. Controlled scenario

The first playable proof is a controlled federal housing-affordability scenario inside the eventual United States game target.

Scenario numbers, political alignments, economic values, and state responses are **test fixtures**, not claims about current real-world conditions.

The player begins as the strategic controller of a newly seated executive administration during a geographically uneven housing-affordability problem.

The opening world includes, at minimum conceptually:

- a federal executive administration controlled by the player;
- a legislature in which the administration cannot command votes;
- finite fiscal room and existing budget obligations;
- a federal administrative institution capable of operating a housing-grant program but with finite capacity;
- multiple state jurisdictions with different incentives and implementation capacity;
- a housing material domain in which construction takes time and local conditions differ;
- an electorate that does not directly observe canonical simulation state; and
- a scheduled election capable of removing the player's administration.

The walking skeleton may use a synthetic miniature federation rather than complete United States content. That fixture must exercise the same semantic loop.

## 3. The governing problem

Housing pressure is high enough to matter politically, but the player does not receive a magic "fix housing" action.

The first federal intervention is a **conditional housing-construction grant program**.

The player may shape a small number of consequential design dimensions, such as:

- federal matching rate;
- state/local eligibility conditions;
- geographic or need-based distribution formula;
- administrative funding;
- reporting, compliance, or enforcement requirements.

These dimensions must create tradeoffs rather than cosmetic variants.

Examples of intended tensions:

- a generous federal match can increase participation but raises fiscal cost;
- strict conditions may improve expected long-run impact while reducing participation or slowing adoption;
- underfunding administration can make the proposal politically cheaper while degrading delivery;
- broad distribution can improve coalition support while diluting material effect;
- concentrated funding can produce stronger local effects while creating political resistance elsewhere.

Exact values and coefficients are not frozen in Commit 1.

## 4. Complete causal loop

A normal successful route must be capable of traversing, without a magical jump:

```text
player diagnosis / agenda
-> institutional intent
-> authority basis
-> proposal
-> bargaining / amendment / legislative decision
-> enacted legal state
-> fiscal authorization / appropriation
-> administrative program
-> state participation or refusal
-> grant/project operations
-> construction pipeline
-> housing stock / affordability pressure
-> measurement and reporting
-> competing political claims
-> population exposure / belief / attribution / salience
-> preference / turnout
-> election
-> office/control transition
-> same persistent world
```

This is an example causal path, not a mandatory universal pipeline for every future governance action.

## 5. Player-facing term arc

### Phase A — inherit and diagnose

The player enters a world with history, existing conditions, uncertainty, and institutional constraints. It is not a clean optimization puzzle.

The player should be able to answer:

- What housing problem appears to exist?
- Where is it most severe according to available evidence?
- What is uncertain or lagged?
- Which federal levers appear institutionally available?
- What political and fiscal constraints are immediately relevant?

The player receives reports and projections, not direct debug truth.

### Phase B — choose an approach

The player defines the administration's housing-grant intent and design posture.

The first decision should involve genuine tradeoffs in impact, participation, cost, administrative burden, and political coalition rather than selecting a stronger or weaker version of one optimal slider.

### Phase C — fight for enactment

Other political actors can:

- support;
- oppose;
- condition support;
- demand amendments;
- delay;
- defect;
- trade support across provisions; or
- kill the proposal.

The player cannot directly set their votes.

The bill that passes may be materially different from the administration's initial intent.

### Phase D — enactment is only the beginning

Passing the proposal creates legal and fiscal state appropriate to what actually passed.

It does not directly:

- create buildings;
- lower rents;
- increase approval;
- force state participation; or
- prove the program can be administered.

The player should immediately see what legal/fiscal victory did and did not accomplish.

### Phase E — implementation and federalism

The responsible federal institution must turn authority and funding into actual administration.

States respond independently within their own incentives, authority, capacity, and politics.

The loop must support at least three materially different implementation responses:

1. willing and capable participation;
2. political or institutional refusal/nonparticipation;
3. nominal participation with weak administrative or delivery capacity.

These are causal world states, not scripted labels.

### Phase F — material consequences take time

Approved programs and grants create operational/project pipelines.

Construction requires time, capacity, and resources before housing stock changes.

Housing outcomes may be:

- delayed;
- geographically uneven;
- smaller than forecast;
- constrained by delivery capacity;
- partially successful; or
- materially successful only after the political window has moved on.

### Phase G — the country does not observe reality perfectly

Material changes become politically relevant through measurements, reports, claims, exposure, belief, attribution, and salience.

The administration may materially succeed while receiving little credit.

It may temporarily receive credit before material delivery catches up.

Official data may lag actual changes.

Competing actors may offer different causal interpretations of the same observed condition.

### Phase H — election and inheritance

The player faces an election shaped by the world and the information environment, not a direct policy-to-happiness table.

If the player loses:

- the controlled administration leaves office at the effective transfer boundary;
- the world does not reset;
- enacted law remains unless later changed;
- remaining fiscal authorizations/obligations remain according to their rules;
- agencies and appointments persist where appropriate;
- unfinished projects remain;
- state participation remains;
- housing stock and construction state remain;
- legal contests/orders remain;
- population beliefs, attribution, salience, and political memory remain; and
- the successor inherits this state.

The V0 run may end active player control after enough successor simulation to demonstrate inheritance. The final product's continuation mode is deferred.

## 6. Required outcome family

Architecture V0 must be capable of representing all of the following without bespoke story branching:

1. **Bill failure** — political coalition never reaches enactment.
2. **Compromise passage** — bill passes after provisions materially change.
3. **Underfunded administration** — legal program exists but administrative resources are inadequate.
4. **State refusal** — eligible jurisdiction declines or blocks participation.
5. **Lawful but incompetent implementation** — authority and money are valid, actors comply, but weak agency/state capacity and material bottlenecks produce disappointing delivery.
6. **Projects not finished before election** — grants and obligations exist while material results remain pending.
7. **Material success with data lag** — actual conditions improve before official measurement fully captures them.
8. **Material success with misattribution** — another actor or institution receives substantial credit.
9. **Messaging advantage without delivery** — political interpretation temporarily runs ahead of material outcomes without changing canonical material truth.
10. **Electoral defeat and succession** — player loses while the created world state persists.
11. **Contested-authority attempt** — an aggressive executive action can be attempted and produce institutional/legal response rather than a generic invalid-command error.

The walking skeleton need not make all eleven routes rich gameplay. It must prove the architecture has legitimate state owners and causal edges for them.

## 7. Decision cadence and legibility

The first term must not become either:

- procedural click spam where the player manually confirms every institutional step; or
- a passive simulator where the player makes one decision and watches eighteen months of bars.

The intended cadence is:

- player makes a strategically meaningful decision;
- routine institutional/material activity advances automatically;
- control returns when new information, changed political feasibility, implementation failure, legal contest, material consequence, or an electoral decision creates a meaningful choice.

The player must be able to inspect why a process is blocked, delayed, revised, succeeding, or failing without seeing omniscient internal state.

## 8. What GL0 intentionally does not require

Governing Loop 0 does not require:

- a complete United States economy;
- all housing policy mechanisms;
- all 50 states at full simulation depth;
- congressional districting;
- primaries;
- full judicial hierarchy or detailed litigation;
- local zoning simulation;
- media-company ownership models;
- lobbying or campaign-finance breadth;
- foreign affairs;
- individual ordinary citizens;
- coups, military loyalty, repression, or comprehensive authoritarian gameplay; or
- a polished commercial UI.

It does require architecture that does not make these foreseeable additions destructive to foundational truth ownership.

## 9. Walking-skeleton proof shape

After architecture review, the first executable proof should be deliberately small:

- one executive administration and officeholder;
- one simplified federal legislature with several political blocs/actors;
- one proposal;
- one fiscal authorization path;
- one federal agency/program;
- a small synthetic set of state jurisdictions with structurally different responses;
- one minimal housing mechanism;
- a tiny aggregate electorate;
- a minimal information/claim path;
- one election; and
- one small legal-contest seam.

The skeleton proves causality, ownership, persistence, and transition semantics. It is not the commercial game.

## 10. Commit-1 review questions

Reviewers should attack:

1. Is housing narrow enough to prove the engine without becoming the engine?
2. Does the term contain recurring meaningful decisions rather than procedure theater?
3. Can failure occur through politics, legality, administration, material capacity, information, or elections without every failure becoming "someone resisted"?
4. Does the loop require any magical policy-to-outcome or policy-to-voter jump?
5. Does the state/federal seam belong in GL0 early enough to prevent bolting federalism on later?
6. Does electoral defeat prove persistence rather than merely ending the scenario?
7. Is any promised outcome impossible to support without a major architecture category not yet acknowledged?
